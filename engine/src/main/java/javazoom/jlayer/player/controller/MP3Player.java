...
package javazoom.jlayer.player.controller;

import java.io.InputStream;
import java.util.concurrent.atomic.AtomicBoolean;

import javazoom.jlayer.player.audio.AudioDevice;
import javazoom.jlayer.player.audio.FactoryRegistry;
...
public class MP3Player
{
	 ...
	 * Creates a new <code>Player</code> instance.
	 */
	public MP3Player(InputStream stream, float gain) throws JavaLayerException
	{
		this(stream, null, gain);
	}

	private MP3Player(InputStream stream, AudioDevice device, float gain) throws JavaLayerException
	{
	...
	public boolean play(int frames, AtomicBoolean isPlaying) throws JavaLayerException
	{
		...
		((JavaSoundAudioDevice) audioDevice).setGain(initialGain);
		while (frames-- > 0 && ret) {
