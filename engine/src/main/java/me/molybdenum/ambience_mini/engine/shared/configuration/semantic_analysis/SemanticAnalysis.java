...
import me.molybdenum.ambience_mini.engine.shared.configuration.messages.SemWarning;
...
import me.molybdenum.ambience_mini.engine.client.core.monitor.decoders.AmDecoder;
...
public record SemanticAnalysis(BaseMusicProvider musicProvider, @Nullable BaseGameStateProvider gameStateProvider)
{
    ...
    public void validate(Config config, ArrayList<Message> messages) {
        ...
        int skipLen = musicProvider.getBasePath().length() + 1;  // +1 for '/' afterward
    ...
    private void Conf(Config config, TypeEnv env, ArrayList<Message> messages) {
        if (config instanceof PlaylistDecl playlistDecl) {
            ...
            // TODO: Handle null gameStateProvider. No error since integrated server on client should just ignore this part.
