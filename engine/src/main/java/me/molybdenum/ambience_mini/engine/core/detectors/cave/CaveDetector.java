...
import me.molybdenum.ambience_mini.engine.core.state.BlockReading;
...
public class CaveDetector<TBlockPos, TVec3, TBlockState>
{
    ...
    protected double computeScore(
        ...
        double averageAirLight = measurements.stream()
                ...
                / measurements.size();
        double averageAirLightRatio = averageAirLight / BaseLevelState.MAX_LIGHT_LEVEL;
        ...
        for (var m : measurements) {
            double maxSkyLightRatio = m.maxSkyLightLevel() / BaseLevelState.MAX_LIGHT_LEVEL;
            ...
            double blockLightRatio = m.blockLightLevel() / BaseLevelState.MAX_LIGHT_LEVEL;
            switch (m.type()) {
                case CAVE -> {
                    maxArtificialLight++;
