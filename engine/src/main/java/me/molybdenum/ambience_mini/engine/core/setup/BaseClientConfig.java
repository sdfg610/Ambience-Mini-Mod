...
public abstract class BaseClientConfig {
    ...
    protected BaseClientConfig() {
        ...
        // Cave detection
        caveScoreRadius = registerIntOption(
        ...
        caveMeasureDistance = registerIntOption(
        ...
        yAxisGranularity = registerIntOption(
        ...
        xAxisGranularity = registerIntOption(
    ...
    private Supplier<Boolean> registerBoolOption(String comment, String name, boolean defaultValue) {
        Supplier<Boolean> config = makeBoolOption(comment, name, defaultValue);
        configs.add(new Pair<>(name, config));
        return config;
    }
    
    private Supplier<Integer> registerIntOption(String comment, String name, int defaultValue, int min, int max) {
        Supplier<Integer> config = makeIntOption(comment, name, defaultValue, min, max);
        configs.add(new Pair<>(name, config));
        return config;
    }
    ...
    public String getConfigsString() {
        List<Pair<String, Object>> pairs = configs.stream()
                .map(pair -> new Pair<>(pair.left(), (Object)pair.right().get()))
                .toList();
        return Utils.getKeyValuePairString(pairs);
    }
}
